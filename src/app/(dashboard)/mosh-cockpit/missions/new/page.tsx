...
function NewMissionForm() {
    ...
    const inputClass = 'w-full px-4 py-2.5 bg-white border border-[#E5E7EB] rounded-xl text-sm text-[#18181B] placeholder:text-[#A1A1AA] focus:outline-none focus:border-[#C4F042] focus:ring-1 focus:ring-[#C4F042]/25'
    const labelClass = 'block text-sm font-medium text-[#18181B] mb-1.5'
        ...
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-3">
                <Link href="/mosh-cockpit/missions"
                    className="w-10 h-10 rounded-xl bg-white border border-[#E5E7EB] flex items-center justify-center hover:bg-[#F4F3EF] transition-colors">
                    <ArrowLeft className="w-4 h-4 text-[#71717A]" />
                </Link>
                <div>
                    <h1 className="text-2xl font-semibold text-[#18181B] flex items-center gap-2">
                        <Film className="w-5 h-5 text-[#C4F042]" />
                        Nouvelle mission
                    </h1>
                    <p className="text-sm text-[#71717A]">Créez une mission et proposez-la directement à un créateur</p>
                </div>
            </div>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                className="bg-white/90 backdrop-blur-sm border border-black/[0.03] rounded-[20px] p-6 space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className="text-xs text-[#A1A1AA] mt-1">Le créateur pourra contre-proposer à la réception.</p>
                    </div>
                    <div>
                        <label className={labelClass}>Usage</label>
                        <div className="flex gap-2">
                            <button type="button" onClick={() => set('isAds', false)}
                                className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-colors ${!form.isAds ? 'bg-[#C4F042]/20 border-[#C4F042]/40 text-[#18181B]' : 'bg-white border-[#E5E7EB] text-[#71717A] hover:bg-[#F4F3EF]'}`}>
                                Organique
                            </button>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <label className="flex items-center gap-2 mt-2 text-xs text-[#71717A] cursor-pointer">
                            <input type="checkbox" checked={form.shootingDateFixed}
                                onChange={e => set('shootingDateFixed', e.target.checked)}
                                className="accent-[#C4F042]" />
                ...
                <div className="flex justify-end gap-3 pt-2">
                    <Link href="/mosh-cockpit/missions"
                        className="px-4 py-2.5 rounded-xl text-sm font-medium text-[#71717A] hover:bg-[#F4F3EF] transition-colors">
                        Annuler
                    </Link>
                    <button onClick={handleSubmit} disabled={submitting}
                        className="px-5 py-2.5 bg-[#18181B] text-[#C4F042] rounded-xl text-sm font-medium hover:bg-[#18181B]/90 transition-colors flex items-center gap-2 disabled:opacity-50">
                        {submitting ? (
