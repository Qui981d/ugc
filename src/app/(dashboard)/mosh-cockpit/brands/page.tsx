                                        ...
                                        >
                                            <td className="px-4 py-2.5 max-w-[280px]">
                                                ...
                                                {brand.profiles_brand?.company_name && (
                                                    <span className="block text-[12px] text-[#9B9B9B] truncate">{brand.full_name}</span>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-4 py-2.5 text-[#6B6B6B] hidden sm:table-cell whitespace-nowrap tabular-nums">
                                    ...
                                    {filteredRequests.map((req) => (
                                        <tr key={req.id} className="hover:bg-[#FAFAF9] transition-colors">
                                            <td className="px-4 py-2.5 max-w-[240px]">
                                                <span className="block font-medium text-[#1A1A1A] truncate">{req.company_name}</span>
                                                <span className="block text-[12px] text-[#9B9B9B] truncate md:hidden">{req.contact_name}</span>
                                            </td>
                                            <td className="px-4 py-2.5 text-[#6B6B6B] hidden md:table-cell max-w-[240px]">
                                                <span className="block truncate">{req.contact_name}</span>
                                                <span className="block text-[12px] text-[#9B9B9B] truncate">
                                                    {req.email}{req.phone ? ` · ${req.phone}` : ''}
                                                </span>
                                            </td>
                                            <td className="px-4 py-2.5 text-[#6B6B6B] hidden xl:table-cell max-w-[260px]">
                                            ...
                                            <td className="px-4 py-2.5 text-right">
                                                <select
                                                    ...
                                                    className="h-8 pl-2 pr-7 bg-white border border-[#E2E2E1] rounded-lg text-[12.5px] text-[#1A1A1A] cursor-pointer focus:outline-none focus:border-[#1A1A1A] focus:ring-2 focus:ring-[#1A1A1A]/15 transition-colors"
                                                >
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center justify-between gap-3 px-4 h-11 border-b border-[#E2E2E1]">
                                <h2 className="text-[13px] font-semibold text-[#1A1A1A]">Nouvelle marque (gérée par MOSH)</h2>
                                <button onClick={() => setShowCreate(false)} className="w-7 h-7 rounded-lg flex items-center justify-center text-[#9B9B9B] hover:bg-[#F4F4F3] hover:text-[#1A1A1A] transition-colors">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="p-4 space-y-3.5">
                                <div>
                                    <label className="text-[12px] font-medium text-[#6B6B6B] mb-1.5 block">Nom de la marque *</label>
                                ...
                                <div>
                                    <label className="text-[12px] font-medium text-[#6B6B6B] mb-1.5 block">Nom du contact (optionnel)</label>
                                    <input value={newBrand.contactName} onChange={e => setNewBrand({ ...newBrand, contactName: e.target.value })}
                                        placeholder="Ex : Marie Dupont"
                                        className="w-full h-9 px-3 bg-white border border-[#E2E2E1] rounded-lg text-[13px] text-[#1A1A1A] placeholder:text-[#9B9B9B] focus:outline-none focus:border-[#1A1A1A] focus:ring-2 focus:ring-[#1A1A1A]/15 transition-colors" />
                                </div>
                                <div>
                                    <label className="text-[12px] font-medium text-[#6B6B6B] mb-1.5 block">Email du contact (optionnel)</label>
                                    <input type="email" value={newBrand.contactEmail} onChange={e => setNewBrand({ ...newBrand, contactEmail: e.target.value })}
                                        placeholder="contact@client.com"
                                        className="w-full h-9 px-3 bg-white border border-[#E2E2E1] rounded-lg text-[13px] text-[#1A1A1A] placeholder:text-[#9B9B9B] focus:outline-none focus:border-[#1A1A1A] focus:ring-2 focus:ring-[#1A1A1A]/15 transition-colors" />
                                    <p className="text-[12px] text-[#9B9B9B] mt-1">Aucun email n&apos;est envoyé. Sert si le client veut se connecter plus tard.</p>
                                </div>
                                {clickupGroups.length > 0 && (
                                    <div>
                                        <label className="text-[12px] font-medium text-[#6B6B6B] mb-1.5 block">Liste ClickUp (optionnel)</label>
                                        <select value={newBrand.clickupListId} onChange={e => setNewBrand({ ...newBrand, clickupListId: e.target.value })}
                                            className="w-full h-9 px-3 bg-white border border-[#E2E2E1] rounded-lg text-[13px] text-[#1A1A1A] cursor-pointer focus:outline-none focus:border-[#1A1A1A] focus:ring-2 focus:ring-[#1A1A1A]/15 transition-colors">
                            ...
                            <div className="px-4 py-3 border-t border-[#E2E2E1] flex items-center justify-end gap-2">
                                <button onClick={() => setShowCreate(false)} className="px-3 h-9 text-[13px] text-[#6B6B6B] hover:text-[#1A1A1A] transition-colors">Annuler</button>
