...
export default function MissionWorkflowPanel({ userRole }: MissionWorkflowPanelProps) {
        ...
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-[#9B9B9B]">Chargement…</p>
                </div>
            </div>
        )
    }
        ...
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="text-center">
                    <div className="w-16 h-16 rounded-lg bg-[#F4F4F3] flex items-center justify-center mx-auto mb-4">
                        <Package className="w-7 h-7 text-[#9B9B9B]" strokeWidth={1.5} />
                    </div>
                    <p className="font-semibold text-[#1A1A1A]">Aucun projet en cours</p>
                    <p className="text-sm text-[#9B9B9B] mt-1 max-w-[260px] mx-auto">
    ...
    const renderColumn = (
        ...
        <div className={`rounded-lg p-4 transition-all ${isActive
            ? darkMode ? 'bg-[#1A1A1A] shadow-lg' : `bg-white border-2 shadow-lg`
            : 'bg-[#F4F4F3]/60 border border-[#E2E2E1]'
            }`}
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white border border-[#E2E2E1] rounded-lg p-5 shadow-sm relative z-20 overflow-visible">
                    ...
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                        ...
                        <div className="relative min-w-0 flex-1 max-w-md">
                            <button
                                ...
                                className="w-full flex items-center justify-between gap-3 bg-[#F4F4F3] hover:bg-[#F2F2F1] rounded-lg px-4 py-3 transition-colors"
                            >
                                <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0 text-left">
                                        ...
                                        <p className="text-[11px] text-[#9B9B9B] truncate">{selectedCampaign.script_type}</p>
                                    </div>
                                </div>
                                <ChevronDown className={`w-4 h-4 text-[#9B9B9B] transition-transform flex-shrink-0 ${projectDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>
                            ...
                            <AnimatePresence>
                                {projectDropdownOpen && (
                                    <motion.div
                                        ...
                                        className="absolute z-50 top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border border-[#E2E2E1] overflow-hidden max-h-64 overflow-y-auto"
                                    >
                                        {campaigns.map(camp => (
                                            <button
                                                key={camp.id}
                                                ...
                                                className={`w-full text-left px-4 py-3 hover:bg-[#F4F4F3] transition-colors flex items-center gap-3 ${camp.id === selectedCampaignId ? 'bg-[#EDEDEC]' : ''}`}
                                            >
                                                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${camp.id === selectedCampaignId ? 'bg-[#1A1A1A]' : 'bg-black/10'}`} />
                                                <div className="min-w-0">
                                                    <p className={`text-sm truncate ${camp.id === selectedCampaignId ? 'font-semibold text-[#1A1A1A]' : 'text-[#6B6B6B]'}`}>
                                                        {camp.title}
                                                    </p>
                                                    <p className="text-[11px] text-[#9B9B9B]">{camp.script_type}</p>
                                                </div>
                                            </button>
                                        ))}
                                    </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
                ...
                <div className="lg:col-span-3 bg-white border border-[#E2E2E1] rounded-lg shadow-sm overflow-hidden">
                    {/* Workflow header */}
                    <div className="px-6 py-4 border-b border-[#E2E2E1] flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-[#9B9B9B]">{userRole === 'creator' ? creatorCompletedCount : completedCount}/{userRole === 'creator' ? CREATOR_PIPELINE.length : WORKFLOW_STEPS.length} étapes</p>
                            </div>
                        </div>
                        {/* Linear progress */}
                        <div className="hidden sm:flex items-center gap-2 flex-1 max-w-[180px] ml-4">
                            ...
                            <span className="text-[10px] text-[#9B9B9B] font-medium">{Math.round(((userRole === 'creator' ? creatorCompletedCount : completedCount) / (userRole === 'creator' ? CREATOR_PIPELINE.length : WORKFLOW_STEPS.length)) * 100)}%</span>
                        </div>
                    </div>
                    ...
                    <div className="p-5">
                        {userRole === 'creator' ? (
                                ...
                                {CREATOR_PIPELINE.map((step, i) => {
                                    ...
                                    return (
                                        <div key={step.type} className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all ${
                                            completed ? 'bg-[#EDEDEC] text-[#1A1A1A]'
                                            : isCreatorAction ? 'bg-[#1A1A1A] text-white font-semibold shadow-sm'
                                            : isActiveStep ? 'bg-[#F4F4F3] text-[#6B6B6B] font-medium'
                                            : 'text-[#9B9B9B]'
                                        }`}>
                                            {completed ? (
                                                <CheckCircle2 className="w-4 h-4 text-[#1A1A1A] shrink-0" />
                                            ) : isActiveStep ? (
                                                <div className={`w-4 h-4 rounded-full border-2 shrink-0 animate-pulse ${isCreatorAction ? 'border-[#1A1A1A]' : 'border-[#6B6B6B]'}`} />
                                            ) : (
                                                <div className="w-4 h-4 rounded-full border border-black/10 shrink-0" />
                                            )}
                                            <span>{step.label}</span>
                                            {isCreatorAction && <span className="ml-auto text-xs bg-[#1A1A1A] text-white px-2 py-0.5 rounded-full font-medium">⚡ À vous</span>}
                                            {isActiveStep && !isCreatorAction && step.owner !== 'creator' && <span className="ml-auto text-xs text-[#9B9B9B]">En attente</span>}
                                        </div>
                                    )
                                })}
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                {/* Owners are told apart by value on a grey ramp, since the
                                    system has no accent hues left to allocate. */}
                                {renderColumn('MOSH', 'M', moshSteps, activeOwner === 'mosh', true, '#1A1A1A')}
                                {renderColumn('Marque', 'B', brandSteps, activeOwner === 'brand', false, '#7A7A7A')}
                                {renderColumn('Créateur', 'C', creatorSteps, activeOwner === 'creator', false, '#4D4D4D')}
                            </div>
                        )}
                    </div>
                </div>
                ...
                {/* ─── BLOCK 3: Messages ─── */}
                <div className="lg:col-span-2 bg-white border border-[#E2E2E1] rounded-lg shadow-sm flex flex-col overflow-hidden"
                ...
                >
                    ...
                    <div className="px-5 py-4 border-b border-[#E2E2E1] flex items-center gap-2.5 flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-[#9B9B9B]">{messages.length} message{messages.length !== 1 ? 's' : ''}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-3 min-h-0">
                        {messages.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-12 text-center h-full">
                                <div className="w-14 h-14 rounded-lg bg-[#F4F4F3] flex items-center justify-center mb-4">
                                    <MessageSquare className="w-6 h-6 text-[#9B9B9B]" strokeWidth={1.5} />
                                </div>
                                <p className="text-sm font-medium text-[#6B6B6B]">Aucun message</p>
                                <p className="text-xs text-[#9B9B9B] mt-1 max-w-[220px]">
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`max-w-[80%] px-4 py-3 text-sm leading-relaxed ${isOwn
                                                ? 'bg-[#1A1A1A] text-white rounded-xl rounded-br-[4px]'
                                                : 'bg-[#F4F4F3] text-[#1A1A1A] rounded-xl rounded-bl-[4px]'
                                                }`}>
                                                {/* Attachment */}
                                                {msg.attachment_url && (
                                                ...
                                                <p className={`text-[10px] mt-1.5 text-right ${isOwn ? 'text-white/40' : 'text-[#9B9B9B]'}`}>
                    ...
                    {/* Input area */}
                    <div className="p-4 border-t border-[#E2E2E1] bg-white/80 flex-shrink-0">
                        {/* Pending file preview */}
                        {pendingFile && (
                            <div className="flex items-center gap-2 mb-2 px-3 py-2 bg-[#F4F4F3] rounded-lg">
                                <Paperclip className="w-3.5 h-3.5 text-[#9B9B9B] flex-shrink-0" />
                                <span className="text-xs text-[#6B6B6B] truncate flex-1">{pendingFile.name}</span>
                                <span className="text-[10px] text-[#9B9B9B]">{(pendingFile.size / 1024).toFixed(0)} KB</span>
                                <button onClick={() => setPendingFile(null)} className="text-[#9B9B9B] hover:text-[#1A1A1A]">
                                    <X className="w-3.5 h-3.5" />
                                </button>
                            </div>
                        )}
                        <div className="flex items-center gap-2">
                            <button onClick={() => fileInputRef.current?.click()} className="w-10 h-10 rounded-lg bg-[#F4F4F3] border border-[#E2E2E1] flex items-center justify-center text-[#9B9B9B] hover:text-[#1A1A1A] hover:bg-[#F2F2F1] transition-all flex-shrink-0">
                            ...
                            <div className="flex-1 relative">
                                <textarea
                                    ...
                                    rows={1}
                                    className="w-full bg-[#F4F4F3] border border-[#E2E2E1] rounded-lg px-4 py-3 text-sm text-[#1A1A1A] placeholder:text-[#9B9B9B] focus:outline-none focus:ring-2 focus:ring-[#1A1A1A]/15 focus:border-[#1A1A1A]/50 focus:bg-white transition-all resize-none overflow-hidden"
                            ...
                            <button
                                ...
                                className="p-3 bg-[#1A1A1A] text-white rounded-lg hover:bg-[#333333] transition-all disabled:opacity-30 disabled:cursor-not-allowed group flex-shrink-0"
                            >
                                {isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4 group-hover:text-[#1A1A1A] transition-colors" />}
                            </button>
                        </div>
                        <p className="text-[10px] text-[#9B9B9B] mt-2 ml-1">
                            <kbd className="px-1.5 py-0.5 bg-[#F4F4F3] rounded text-[9px] font-mono">Entrée</kbd> envoyer · <kbd className="px-1.5 py-0.5 bg-[#F4F4F3] rounded text-[9px] font-mono">Maj+Entrée</kbd> retour à la ligne
                        </p>
                    </div>
